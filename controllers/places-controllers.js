...
const createPlace = async (req, res, next) => {
  ...
  try {
    ...
    const error = new HttpError(
      "Please try again, creating place failed asdfasdf",
      500
    );
    return next(error);
  }
  ...
  if (!user) {
    const error = new HttpError(
      "Could not find user for the id, creating place failed",
      500
    );
    return next(error);
  }
  ...
  try {
    ...
    const error = new HttpError("Please try again, creating failed", 500);
    return next(error);
  }
